...
def plotting(inputs, title, xmin, ymin):

    #Taking info from the .mat file 'inputs'
    dat = scipy.io.loadmat(inputs)
    u_pred = dat['sol']
    training_data = dat['data']


    #retrieving  known solution from burgers_shock.mat
    ...
    if title == 'solution':
        ...
        ax.set_title('$u(t,x) - u_{\text{pred}}(t,x)$', fontsize=10)
    else:
        print('specify solution or difference')
