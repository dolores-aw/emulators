...
def main():
    ...
    for k in range(0,10):
       #declaring, training model
        inputs = interiorburgerslambda.prepare_nn_inputs_burgers('burgers_shock.mat', N_u, N_f, N_f2, m, random_seed = input_seed, debugging=False)
        model = burgersraissilambda.PhysicsInformedNN(lam, inputs.X_u_train,  inputs.u_train, inputs.X_f_train, burgers_layers, lb, ub, inputs.nu, X_star, N_u, N_f, N_f2, m)
        start_time = time.time()
        #if N_f > 0:
        #model.load_weights_and_biases('wab/weights_and_biases_%s_%s_%s_%s.npz' % (N_u, N_f - 1, typen, args.epochs))
        losses = model.train(args.epochs, args.data_loc, N_u, N_f, N_f2, m, args.base_plot_dir)
        ...
        scipy.io.savemat('sols/solution_data_%s.mat' % m, solution_dict)
        errors.append(error)
        #             loss, weights, biases
