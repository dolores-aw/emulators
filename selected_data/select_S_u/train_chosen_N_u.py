...
import pandas as pd
import numpy as np
import interiorburgersN_u, argparser_raissi, plotting, burgersraissilambda
import scipy.io
import pickle
def main(): #first use find_param, select_param to identify needed hyperparameter from front of MSE_F
    ...
    N_f2 = 0 # num. of points of S_f to choose within m of x = 0
    m  = 1 # distance from x = 0 to select certain points
    N_u2 = 25
    typen = 'N_u'
    ...
    for k in range(0,15):
       #declaring, training model
        inputs = interiorburgersN_u.prepare_nn_inputs_burgers('burgers_shock.mat', N_u, N_f, N_u2, N_f2, m, typen, debugging=False)
        ...
        solution_dict = {'sol' : u_pred, 'u_data' : inputs.X_u_train, 'u_data2' : inputs.X_u_train2, 'f_data' : inputs.X_f_train,  }
        
        scipy.io.savemat('chosen_data/sols/N_u/solution_data_%s_%s_%s_%s.mat' % (m, N_u2, typen, error), solution_dict)
        errors.append(error)
    ...
    d[typen] = dictionary
    with open('data_comparisonN_u.p', 'wb') as fp:
