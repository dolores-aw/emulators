...
class NNInputs_burgers(object):
    def __init__(self, X_u_train, u_train, X_f_train, X_u_train2, u_train2, lb, ub, nu, X_star, x, t, exact):
...
def prepare_nn_inputs_burgers(data_loc, N_u, N_f, N_u2, N_f2, m, typen, debugging=False):
    ...
    #with open('training_sets_N_u.p', 'rb') as fp:
    ...
    #u_train = np.vstack((u_train, u_train2))
    #X_f_train = np.vstack((X_f_train, X_u_train))
    return NNInputs_burgers(X_u_train, u_train, X_f_train, X_u_train2, u_train2, lb, ub, nu, X_star, x, t, Exact)
